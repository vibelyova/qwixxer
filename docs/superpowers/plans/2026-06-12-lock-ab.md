...
Note: the z-vs-50% on ties — ties count as losses in `win_rate` here, matching the production bench's unique-winner convention. For a symmetric head-to-head A/B this is fine (both sides tie equally); state it in the output if you prefer, but keep the convention.
